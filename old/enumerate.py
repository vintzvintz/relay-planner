...
def _collect_configs(constraints, min_score, max_configs, run_ts):
    ...
    try:
        while True:
            ...
            score, n_binomes, n_solos, km_solos, n_flex, n_fixes, km_flex = relay_sol.stats()
            km_flex_str = f" ({km_flex:.1f} km)" if km_flex else ""
            print(f"  Config {config_idx} trouvée — score:{score} binomes:{n_binomes} solos:{n_solos} ({km_solos:.1f} km) flex:{n_flex}{km_flex_str}")
