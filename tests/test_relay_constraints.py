...
class TestInit:
    ...
    def test_repos_jour_segs(self, c):
        # 7h * 10 km/h / 100 km * 10 segs = 7 → ceil(7) = 7
...
class TestAddRelay:
    def test_single_relay(self, c):
        ...
        assert c.runners_data["Alice"].relais[0].size == {3}

    def test_nb_relay(self, c):
        c.new_runner("Alice").add_relay({3}, nb=4)
        assert len(c.runners_data["Alice"].relais) == 4

    def test_multiple_add_relay_calls(self, c):
        c.new_runner("Alice").add_relay({3}).add_relay({5}, nb=2)
        assert len(c.runners_data["Alice"].relais) == 3
        assert c.runners_data["Alice"].relais[0].size == {3}
        assert c.runners_data["Alice"].relais[1].size == {5}

    def test_relay_sizes_property(self, c):
        c.new_runner("Alice").add_relay({3}).add_relay({5})
        assert c.relay_sizes["Alice"] == [3, 5]

    def test_flex_relay_nominal_size_is_max(self, c):
        c.new_runner("Alice").add_relay({3, 4, 5})
        assert c.relay_sizes["Alice"] == [5]

    def test_pinned_relay(self, c):
        c.new_runner("Alice").add_relay({3}, pinned=2)
...
class TestSharedRelay:
    ...
    def test_shared_relay_size_propagated(self, c):
        shared = c.new_relay({9})
        c.new_runner("Alice").add_relay(shared)
        c.new_runner("Bob").add_relay(shared)
        assert c.runners_data["Alice"].relais[0].size == {9}
        assert c.runners_data["Bob"].relais[0].size == {9}
...
class TestAddMaxBinomes:
    ...
    def test_add_max_binomes_enregistre(self, c):
        alice = c.new_runner("Alice").add_relay({3})
        bob = c.new_runner("Bob").add_relay({3})
    ...
    def test_add_max_binomes_multiple(self, c):
        alice = c.new_runner("Alice").add_relay({3})
        bob = c.new_runner("Bob").add_relay({3})
        carol = c.new_runner("Carol").add_relay({3})
        c.add_max_binomes(alice, bob, nb=1)
...
# ---------------------------------------------------------------------------
# set_max_same_partenaire / max_same_partenaire
# ---------------------------------------------------------------------------
...
class TestMaxSamePartenaire:
    ...
    def test_set_max_same_partenaire_stores_value(self, c):
        c.new_runner("Alice").set_max_same_partenaire(2).add_relay({3})
        assert c.runners_data["Alice"].max_same_partenaire == 2

    def test_set_max_same_partenaire_is_chainable(self, c):
        builder = c.new_runner("Alice")
        result = builder.set_max_same_partenaire(1)
        assert result is builder

    def test_set_max_same_partenaire_independent_per_runner(self, c):
        c.new_runner("Alice").set_max_same_partenaire(1).add_relay({3})
        c.new_runner("Bob").add_relay({3})
...
class TestSegmentUtils:
    ...
    def test_is_night_true_at_2h(self, c):
        # seg à 2h du matin : heures_depuis_départ = 11h → seg 11 (si 1h/seg)
        # 15 + 11 = 26 → 26 % 24 = 2h → nuit
