...
from solution_formatter import _save_csv
...
MAX_PER_CONFIG = 20       # placements max par configuration de binômes
MAX_CONFIGS = 25          # configs max en phase 1 (None ou 0 = pas de limite)
SOLVER_NUM_WORKERS = 8


def _save_one(relay_model, solver, run_ts, config_idx, place_idx):
    os.makedirs(OUTDIR, exist_ok=True)
    base = os.path.join(OUTDIR, f"run_{run_ts}_config_{config_idx:03d}_place_{place_idx:02d}")

    relais_list = relay_model.parse_relais(solver)
    _save_csv(relais_list, base + ".csv")

    print(f"  → {base}.csv")


def _collect_configs(relay_model, solver, optimal_score, run_ts):
    ...
    try:
        while True:
            ...
            _save_one(relay_model, solver, run_ts, config_idx, 0)
            ...
            active_bvs = [bv for key, bv in relay_model.same_relay.items() if key in active_keys]
            relay_model.model.add_bool_or([~b for b in active_bvs])
...
def enumerate_solutions():
    ...
    if OPTIMAL_BINOMES_NUM is not None:
        ...
        relay_model.model.clear_objective()

    run_ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    print(f"Run timestamp : {run_ts}")

    print( f"Étape 1 : collecte des configurations de binômes (limite={MAX_CONFIGS}) ...")
    #print("  Ctrl-C (1 fois) pour stopper la phase 1 et passer à la phase 2")
    configs = _collect_configs(relay_model, solver, optimal_score, run_ts)
    ...
    print(f"Étape 2 : énumération des placements par configuration (limite = {MAX_PER_CONFIG})...")
    n_total = 0
    ...
    try:
        for config_idx, active_keys in enumerate(configs, start=1):
            ...
            relay_model_f = build_model_fixed_config(active_keys, optimal_score)
            ...
            status = solver_f.solve(relay_model_f.model)
            if status not in (cp_model.OPTIMAL, cp_model.FEASIBLE):
                print("  Aucun placement trouvé pour cette configuration.")
                continue

            n_total += 1
            _save_one(relay_model_f, solver_f, run_ts, config_idx, 1)

            def _add_cut(relay_model_f, solver_f, place_idx):
                """Ajoute une contrainte excluant le placement courant du solveur."""
                start_vals = {r: [solver_f.value(relay_model_f.start[r][k]) for k in range(len(RUNNERS_DATA[r].relais))]
                              for r in RUNNERS}
                cut_lits = []
                for r in RUNNERS:
                    for k in range(len(RUNNERS_DATA[r].relais)):
                        val = start_vals[r][k]
                        b = relay_model_f.model.new_bool_var(f"cut_{place_idx}_{r}_{k}")
                        relay_model_f.model.add(relay_model_f.start[r][k] != val).only_enforce_if(b)
                        relay_model_f.model.add(relay_model_f.start[r][k] == val).only_enforce_if(~b)
                        cut_lits.append(b)
                relay_model_f.model.add_bool_or(cut_lits)

            _add_cut(relay_model_f, solver_f, 1)
            n_places = 1
            for place_idx in range(2, MAX_PER_CONFIG + 1):
                ...
                n_total += 1
                n_places += 1
                _save_one(relay_model_f, solver_f, run_ts, config_idx, place_idx)
                _add_cut(relay_model_f, solver_f, place_idx)
