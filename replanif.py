...
Scénario de replanification : même configuration de coureurs qu'exemple.py,
à utiliser avec `python replanif.py replanif --ref <solution.json>`.
"""
...
# les bornes min/max conditonnent les possibilité de duo, en complément compat_matrix.
...
(c
    ...
    #c.add_pause(wp=154, duree_heures=0)
    ...
    #.add_no_solo( c.interval_time(end_h=7.0, end_j=1))  # aucun solo le 1r jour
...
quentin_martin = c.new_shared_relay(R15)  # pour tester
dispo_quentin = c.interval_time(start_h=22.0, start_j=1)

# j'ai essayé de forcer une partie des relais quentin la nuit 
# mais le problème devient infaisable à cause des compatibilités limitées avec les autres coureurs dispos vendredi
#dispo_quentin_nuit = c.interval_time(start_h=22.0, start_j=1, end_h=8, end_j=2)  # premier double relais priorisé la nuit
#dispo_quentin_jour = c.interval_time(start_h=5.0, start_j=2)
(quentin
    #.set_options(solo_max=0)
    .set_options(repos_jour=0.5, repos_nuit=0.5)   # pas de repos pour les braves car infaisable avec repos standard
    
    # 2 x 15 km avec duo imposé sur le second ( pour tester )
    .add_relay(R15_F, quentin_martin, window=dispo_quentin)

    # 3 x 10km
    .add_relay(R10_15, R10_15, R10_15, window=dispo_quentin)
)
...
(martin
    .add_relay(quentin_martin)    # jumelage pour tester
    .add_relay(R15_F)
    .add_relay(R15_F)
    .add_relay(R15_F)
)
...
(arthur
    ...
    #.add_relay(R10, start_km=0)   # premier relais forcé
...
(oscar
    ...
    .add_relay(arthur_oscar_1, window=slot_30k_1)
    .add_relay(arthur_oscar_2, window=slot_30k_2)
    #.add_relay(R10, end_wp=c.last_point)  # forcé sur derniers segments
...
dispo_clemence1 = c.interval_time(end_h=23, end_j=0)    # debut implcite
